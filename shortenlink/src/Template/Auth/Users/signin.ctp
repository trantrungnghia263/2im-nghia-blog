...
<div class="social-auth-links text-center">
    ...
    <?php if ((bool) get_option('social_login_facebook', false)) : ?>
        <a class="btn btn-block btn-social btn-facebook" href="<?= $this->Url->build(['controller' => 'Users', 'action' => 'signin', '?' => ['provider' => 'Facebook']]); ?>">
            <i class="fa fa-facebook"></i> <?= __("Sign in with Facebook") ?>
        </a>
    <?php endif; ?>

    <?php if ((bool) get_option('social_login_twitter', false)) : ?>
        <a class="btn btn-block btn-social btn-twitter" href="<?= $this->Url->build(['controller' => 'Users', 'action' => 'signin', '?' => ['provider' => 'Twitter']]); ?>">
            <i class="fa fa-twitter"></i> <?= __("Sign in with Twitter") ?>
        </a>
    <?php endif; ?>

    <?php if ((bool) get_option('social_login_google', false)) : ?>
        <a class="btn btn-block btn-social btn-google" href="<?= $this->Url->build(['controller' => 'Users', 'action' => 'signin', '?' => ['provider' => 'Google']]); ?>">
